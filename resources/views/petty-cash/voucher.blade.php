    ...
    <div class="p-8 space-y-6">
        ...
        <div class="space-y-6">
            ...
            <div class="pt-6 border-t border-gray-100 flex justify-between items-center text-[11px] text-gray-400">
                ...
                <p>{{ $pettyCash->proofs && $pettyCash->proofs->count() > 0 ? 'Page 1 of 2' : 'Page 1 of 1' }}</p>
            </div>
        </div>

        <!-- ================= PAGE 2: ATTACHED PROOFS OF EXPENDITURE ================= -->
        @if($pettyCash->proofs && $pettyCash->proofs->count() > 0)
        <div class="print-page-break pt-8 border-t-2 border-dashed border-gray-200">
            <!-- Header Banner for Proofs Page -->
            <div class="flex justify-between items-center border-b-2 border-gray-900 pb-4 mb-6">
                <div>
                    ...
                    <p class="text-xs text-gray-500 font-bold uppercase mt-0.5">ATTACHED PROOFS OF EXPENDITURE</p>
                </div>
                <div class="text-right">
                    <h1 class="text-lg font-mono font-bold text-gray-800">{{ $pettyCash->reference_number }}</h1>
                    <span class="text-xs text-gray-500 font-semibold">Total Attachments: {{ $pettyCash->proofs->count() }}</span>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                @if($nonImages->count() > 0)
                    <div class="mb-4">
                        <h4 class="text-xs font-bold text-gray-700 uppercase mb-2">Attached Documents:</h4>
                ...
                @if($images->count() > 0)
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        @foreach($images as $proof)
                            <div class="bg-gray-50/70 border border-gray-200 rounded-xl p-4 text-center space-y-3 avoid-break">
                                <div class="flex justify-between items-center text-xs font-bold text-gray-700 px-1 border-b border-gray-200 pb-2">
                                    <span class="truncate max-w-[220px]" title="{{ $proof->file_name }}">{{ $proof->file_name }}</span>
                                ...
                                <div class="bg-white rounded-lg p-2 border border-gray-200 flex items-center justify-center min-h-[250px]">
                                    <img src="{{ $getProofUrl($proof->file_path) }}" alt="{{ $proof->file_name }}" class="w-full max-h-[450px] object-contain rounded-md proof-img mx-auto">
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="pt-6 mt-8 border-t border-gray-100 flex justify-between items-center text-[11px] text-gray-400">
                <p>Generated by Loops Finance on {{ date('d M Y, h:i A') }}</p>
                <p>Page 2 of 2</p>
            </div>
        </div>
